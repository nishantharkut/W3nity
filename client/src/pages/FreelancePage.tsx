...
import { Search, Filter, Plus, Grid, List, TrendingUp, Star, DollarSign } from 'lucide-react';
...
const FreelancePage = () => {
  ...
  const popularSkills = ['React', 'TypeScript', 'Node.js', 'Python', 'Figma', 'Solidity', 'Web3'];
  ...
  const filteredGigs = gigs.filter(gig => {
    const matchesSearch = gig.title.toLowerCase().includes(debouncedSearch.toLowerCase()) ||
  ...
  const toggleSkill = (skill: string) => {
    ...
    toast({
      ...
      description: `${skill} ${selectedSkills.includes(skill) ? 'removed from' : 'added to'} filter`,
    });
  };
  ...
  const handleCategoryChange = (category: string) => {
    ...
    toast({
      ...
      description: category === 'all' ? 'Showing all categories' : `Filtering by ${category.replace('-', ' ')}`,
    });
  };
    ...
    <div className="space-y-6">
      ...
      <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4">
        <div>
          ...
          <p className="text-muted-foreground">Discover amazing opportunities from top clients worldwide</p>
          ...
          <CardTitle className="flex items-center justify-between">
            ...
            {(selectedSkills.length > 0 || selectedCategory !== 'all' || searchQuery) && (
              <Button variant="outline" size="sm" onClick={clearFilters}>
                Clear All ({selectedSkills.length + (selectedCategory !== 'all' ? 1 : 0) + (searchQuery ? 1 : 0)})
              </Button>
            )}
          </CardTitle>
        </CardHeader>
